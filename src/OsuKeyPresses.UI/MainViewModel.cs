...
public class MainViewModel : INotifyPropertyChanged
{
    ...
    public Bitmap[] ModImagePaths => ScoreInfo?.APIMods
        ...
        .Select(x => new Bitmap(AssetLoader.Open(new Uri($"avares://OsuKeyPresses.UI/Assets/Mods/{x.Acronym}.png"))))
        .ToArray() ?? [];

    public Bitmap? RulesetImage =>
        ScoreInfo is not null
            ? new Bitmap(AssetLoader.Open(
                new Uri($"avares://OsuKeyPresses.UI/Assets/Rulesets/{ScoreInfo.Ruleset.ShortName}.png")))
            : null;
